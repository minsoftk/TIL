...
### BEM이란?
![Block Element Modifier ](https://velog.io/@ylem76/BEM)
<br/>
<br/>
<br/>
...
-- class 적용
.tomato{
	.tomato는 class = "tomato"와 같다.
}
</style>
```
* class는 값을 여러개 가질 수 있다. id는 유일한 값을 가진다. 따라서 id로 묶을 수 없을땐 class로 여러개를 묶어준다.
...
* Inline은 width, height가 존재하지 않는다. Inline은 Box가 아니다.
* Block은 width, heigth가 존재한다.
* Box모델은 padding, border, margin이 존재한다.
...
```
> Collapsible margins
* body안의 div가 하나 들어갔을때 경우, div의 경계와 body의 경계가 일치한다면 위 아래의 경우의 margin은 일치한다. 즉, 하나의 margin 설정으로 변한다. 즉, body와 div의 margin설정을 따로 할 수 없다. 그렇다면 body안에서의 div가 경계선 안으로 오게 만드려면 어떻게 해야하는가? 아래와 같은 상황일때..
-> 이를 해결하기 위해 padding이 존재한다.

![](https://github.com/MinsoftK/TIL/blob/master/HTML-CSS-JS/img/3.5.png?raw=true)
<br/>
<br/>
...
* 정리하면 inline은 width, height를 가지지 않는다. 그래서 위, 아래에 margin을 가질 수 없다. padding은 사방으로 가능하다. margin은 좌우로만 가질 수 있다. 위아래 margin을 가지게 하려면 block으로 변경해야 한다.
